...
import { GouvDataService } from '../core-data/services/gouv-data.service';
import { Adresse } from '../core-data/models/adresse';
...
export class ToolbarComponent {
  ...
  @Output() selectedAdresse = new EventEmitter<any>();
  ...
  constructor( public gouvDataSvc: GouvDataService) {
  ...
  getStatesAsObservable(token: string): Observable<any> {
    ...
    return of(
      this.statesComplex.filter((state: any) => {
        return query.test(state.libelle);
      })
    );
  }
  ...
  typeaheadOnSelect(e: TypeaheadMatch): void {
    ...
    let adresse = this.statesComplex.find((adr)=> adr.libelle === e.value)
    this.selectedAdresse.emit(adresse);

  }

  changeSelect(){
    if(this.asyncSelected){
        this.gouvDataSvc.apiGouvCall(this.asyncSelected).subscribe(res => { this.statesComplex = res
